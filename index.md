...
In order to launch the change, we'd want the change to have these effects:
- reduce the number of enrollments, so that the users, who would leave the course between enrollment and payment in case they enroll, are filtered out before enrollment.
- not reduce the number of payments

In terms of metrics this would be:
- gross conversion is reduced
- net conversion is not reduced (either is incrased or stays the same)

An example of a desirable situation:<br>
Control: 100 click, 20 enroll, 10 pay<br>
Experiment: 100 click, 10 enroll, 10 pay.<br>
In the control group we have 10 enrolled users quit before payment, but in the experiment group these users didn't enroll.<br>
Here, gross conversion is reduced from 20% to 10%, but net conversion is still 10%, meaning that we didn't lose paying users.

__Unused metrics:__<br>
Number of user-ids: this metric will change, but we only care about relative change to the number of clicks, not the absolute value.<br>
...
- Practical significance: The lower bound of the interval passes the practical significance boundary of 0.75%, so there's a possibility that the change actually reduces the net conversion by a practical boundary, which is not what we want.
...
__Summary__<br>
I didn't use Bonferroni correction, as I need both metrics to match my alternative hypothesis in order to launch the change.

For both metrics, effect size test and sign test agree.
<!-- State whether you used the Bonferroni correction, and explain why or why not. If there are any discrepancies between the effect size hypothesis tests and the sign tests, describe the discrepancy and why you think it arose. -->

###Recommendation
To launch the change, we need net conversion to not decrease, and gross conversion to decrease.<br>
The gross conversion decreased in the experiment, but there is not enough confidence that the net conversion did not decrease.<br>
Thus, with the current information, the change should not be launched.

We can conclude that the time availability prompt successfully filters out some of the users who would leave before payment, but we are unsure whether it also filters out paying users.
...
Invariant metric: number of user-ids. It shouldn't change as we are only altering the course after the enrollment.
